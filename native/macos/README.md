...
The Release workflow reads `TOKEN_MONITOR_APP_GROUP`, `TOKEN_MONITOR_WIDGET_BUNDLE_ID`, `TOKEN_MONITOR_WIDGET_KIND`, and `DEVELOPMENT_TEAM` from GitHub Actions repository variables and fails before building when any is missing. Signing certificates and notarization credentials remain in Actions secrets.
