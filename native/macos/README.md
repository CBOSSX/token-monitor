...
The Release workflow reads `TOKEN_MONITOR_APP_GROUP`, `TOKEN_MONITOR_WIDGET_BUNDLE_ID`, `TOKEN_MONITOR_WIDGET_KIND`, and `DEVELOPMENT_TEAM` from GitHub Actions repository variables and fails before building when any is missing. For a `group.*` App Group, store the main-app and extension profiles as the base64-encoded Actions secrets `TOKEN_MONITOR_APP_PROVISIONING_PROFILE_BASE64` and `TOKEN_MONITOR_WIDGET_PROVISIONING_PROFILE_BASE64`; the workflow decodes them into temporary files and exports the profile paths expected by the build. A Team-prefixed App Group does not use those profile secrets. Signing certificates and notarization credentials remain in Actions secrets.
